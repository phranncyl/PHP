...
ob_start();
  ...
  class Database{
    ...
    public function create_user($name, $email, $username, $password, $dir_photo, $birth_date){
      //$dir_photo = $this->userphoto();

      $sqlinsert = "INSERT INTO users (name, email, username, password, dir_photo,status, update_date, birth_date) VALUES ('$name', '$email', '$username', '$password', '$dir_photo', 'A', now(), '$birth_date')";
      $res = mysqli_query($this->connection, $sqlinsert);
    ...
    public function update_user($id_user, $name, $username, $email, $password, $dir_photo, $birth_date){
      $sql = "UPDATE users SET name = '$name', email = '".$email."', password = '".$password."', username = '".$username."', dir_photo = ifnull('".$dir_photo."', 'img/profile.jpg'), birth_date = '".$birth_date."' WHERE id_user = '".$id_user."';";
      echo($sql);
      ...
      if ($res) {
          ...
          $_SESSION['msg'] = $res."<p style='color:red;'>User not updated</p>";
      }
  }
